...
const MCompatible = () => {
  ...
  const show = (value: number) => {value == phone ? setPhone(0): setPhone(value)}
  const showTablet = (value: number) => {value == tablet ? setTablet(0): setTablet(value)}

  return (
    <>
      <Header headerType={HEADER_TYPE.DETAIL} headerTitle='Device Compatible    ' />
      <div className='content-wrapper p-6 min-h-[calc(100vh-6rem)]'>
        <div className="keterangan-section">
            <p className='mb-4 text-sm font-[350]'>
                To use an eSIM, your device must be carrier-unlocked and eSIM-compatible. Compatibility is only set by the device manufacturer, or in some cases, by the local carrier of the country in which you bought your phone (typically, if you have a contract with a carrier that finances your phone, your device would be locked and cannot use eSIM).
            </p>
            <p className='mb-4 text-sm font-[350]'>
                Below is a list of some brands of iOS and Android devices that are compatible using eSIM.
            </p>
            <p className='mb-4 text-sm font-[350]'>
                If your device is not listed below and you would like to confirm if it supports eSIM, 
                please feel free to <a className='text-[#FFDE95]' href="">Contact Us.</a>
            </p>
        </div>
                ...
                <div className='tab-content pt-4 tab-space'>
                  ...
                  >
                    <div className="listPhone border border-[#E2DFDF] bg-white rounded-[9px]">
                        <div onClick={() => show(1)} className='cardContent flex justify-between border border-t-[#E2DFDF] px-3 py-3 items-center'>
                            <h6 className='text-[16px] font-medium'>Android</h6>
                            {phone == 1 ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                        </div>
                        <div className={`contentHide ${phone == 1 ? 'block': 'hidden'} text-left p-4`}>
                        ...
                        <div onClick={() => show(2)} className='cardContent flex justify-between border border-t-[#E2DFDF] px-3 py-3 items-center'>
                            <h6 className='text-[16px] font-medium'>iOS</h6>
                            {phone == 2 ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                        </div>
                        <div className={`contentHide ${phone == 2 ? 'block': 'hidden'} text-left p-4`}>
                            <ol className='list-disc list-inside [&>*:first-child]:list-none [&>*:first-child]:text-sm [&>*:first-child]:font-extrabold text-sm font-normal mb-4'>
                                <li>Xiaomi</li>
                                <li>Xiaomi 14 Global Version</li>
                                <li>Xiaomi 13, 13 Lite, 13 Pro</li>
                  ...
                  >
                    <div className="listTablet border border-[#E2DFDF] bg-white rounded-[9px]">
                        <div onClick={() => showTablet(1)} className='cardContent flex justify-between border border-t-[#E2DFDF] px-3 py-3 items-center'>
                            <h6 className='text-[16px] font-medium'>Samsung</h6>
                            {tablet == 1 ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                        </div>
                        <div className={`contentHide ${tablet == 1 ? 'block': 'hidden'} text-left p-4`}>
                            <ol className='list-disc list-inside [&>*:first-child]:list-none [&>*:first-child]:text-sm [&>*:first-child]:font-extrabold text-sm font-normal mb-4'>
                                <li>Xiaomi</li>
                                <li>Xiaomi 14 Global Version</li>
                                <li>Xiaomi 13, 13 Lite, 13 Pro</li>
                        ...
                        <div onClick={() => showTablet(2)} className='cardContent flex justify-between border border-t-[#E2DFDF] px-3 py-3 items-center'>
                            <h6 className='text-[16px] font-medium'>Ipad</h6>
                            {tablet == 2 ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                        </div>
                        <div className={`contentHide ${tablet == 2 ? 'block': 'hidden'} text-left p-4`}>
                            <ol className='list-disc list-inside [&>*:first-child]:list-none [&>*:first-child]:text-sm [&>*:first-child]:font-extrabold text-sm font-normal mb-4'>
                                <li>Xiaomi</li>
                                <li>Xiaomi 14 Global Version</li>
                                <li>Xiaomi 13, 13 Lite, 13 Pro</li>
